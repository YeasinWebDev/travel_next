...
import { Eye, Pencil, Trash2, Calendar, Users, MapPin, MoreHorizontal, XCircle, CheckCircle, Clock, AlertCircle } from "lucide-react";
...
function DashboardTripsTable({ allTrips, onView }: DashboardTripsTableProps) {
  ...
  return (
    <div className={`rounded-md border mt-5 ${state === "collapsed" ? "md:w-full" : "md:w-[67%] lg:w-[87%] xl:w-full"}`}>
                ...
                <TableRow key={trip._id} className="hover:bg-gray-50/50">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <span className="text-sm">{trip.creator?.name || "Unknown"}</span>
                    </div>
                  </TableCell>
      ...
      <DashboardTripsUpdateModal
        open={EditModalOpen}
        onOpenChange={setEditModalOpen}
        trip={{ _id: targetTrip?._id!, title: targetTrip?.title!, currentStatus: targetTrip?.status! }}
      />
    </div>
  );
}

export default DashboardTripsTable;
